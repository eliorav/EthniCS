...
def render_solver_by_pools(fig, row, col, df, metric, sparsity_ratio=None, ethnicity_num=None, showlegend=True, log_y=True):
    ...
    for trace in traces:
        ...
        if trace.name == ETHNICS_SOLVER:
            ...
            fig.add_trace(go.Scatter(x=trace.x, y=trace.y, mode="markers", marker=dict(symbol="square-open", size=12, color=solver_to_color[ETHNICS_SOLVER], line=dict(width=4, color=solver_to_color[ETHNICS_SOLVER])), name=f"<b>{ETHNICS_SOLVER}</b>", showlegend=showlegend), row=row, col=col)
        else:
...
def render_heatmap(fig, row, col, coloraxis, df, metric, solver=ETHNICS_SOLVER, n=1024):
    ...
    temp_df2 = pd.DataFrame(columns=temp_df["m/n"].unique(), index=temp_df.k.unique())
...
def get_exp_scatter_plot(fig, row, col, x, xhat, title, color=None):
    fig.add_trace(go.Scatter(x=x, y=xhat, name=title, mode="markers", marker_color=color), row=row, col=col)

    fig.add_trace(go.Scatter(x=x, y=x, line_color="black", showlegend=False, line=dict(dash="dot"), name="x"), row=row, col=col)
    return fig
...
def render_by_ethnicity_num(df, metric, ethnicity_names, ethnicities_to_full_name_mapping, ethnicity_num1, ethnicity_num2, num_of_pools, log_y1=True, log_y2=True, title_text="", n=1024):
    ...
    fig, sparsity1 = render_solver_by_pools(fig, row=1, col=1, df=df, metric=metric, ethnicity_num=ethnicity_num1, log_y=True)
    fig, sparsity2 = render_solver_by_pools(fig, row=1, col=4, df=df, metric=metric, ethnicity_num=ethnicity_num2, log_y=True, showlegend=False)

    fig.layout.annotations[0].text = f"<b>{ethnicities_to_full_name_mapping[ethnicity_names[ethnicity_num1]]}</b><br>{sparsity1}"
...
def render_grid_error_bar_by_non_zero(df, title_text=""):
    fig = make_subplots(rows=4, cols=2, subplot_titles=[f"m={m}" for m in range(64, 513, 64)])
...
def render_grid_error_bar_by_num_of_pools(df, range=np.arange(0.01, 0.11, 0.01), title_text=""):
    ...
    fig = make_subplots(rows=rows, cols=2, subplot_titles=[f"k={round(k, 2)}" for k in range])
