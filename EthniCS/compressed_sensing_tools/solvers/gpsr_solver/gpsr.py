...
def gpsr(
    ...
    for k_iter in range(iter_max):
        ...
        # compute alpha0
        bg_k[:sig_len] = np.matmul(trans_t_trans, g_k[:sig_len] - g_k[sig_len:])
        ...
        while True:
            ...
            bw_k[:sig_len] = np.matmul(trans_t_trans, w_k[:sig_len] - w_k[sig_len:])
...
def gpsr_bb(
    ...
    # Bz
    b_z_k[:sig_len] = np.matmul(trans_t_trans, z_k[:sig_len] - z_k[sig_len:])
    ...
    for k_iter in range(iter_max):
        ...
        b_delta_k[:sig_len] = np.matmul(
            trans_t_trans, delta_k[:sig_len] - delta_k[sig_len:]
        )
